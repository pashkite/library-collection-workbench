...
최초 전체 수집은 `tools/python-holdings-fetcher`의 Python GUI에서 실행합니다. 이 도구가
`public/data/holdings.latest.json`, `public/data/holdings.meta.json`, 사람이 확인할 수 있는
`holdings.xlsx`를 생성하고, 미리보기와 사용자 확인 후 로컬 Git 저장소 `public/data`에 반영합니다.

GitHub Actions의 `Update holdings JSON` 워크플로는 매일 실행되는 증분 확인용입니다. 기본 실행에서
`type=ALL` 전체 수집을 호출하지 않고, `startDt`/`endDt` 날짜 조회로 최근 7일 등록자료만 겹쳐 확인한 뒤
새 자료가 있을 때만 기존 JSON에 병합합니다.
...
- `DAILY_LOOKBACK_DAYS`: Actions Variables에 설정하는 일일 조회 겹침 기간입니다. 없으면 `7`일입니다.

수동 전체 갱신은 GitHub Actions가 아니라 Python GUI에서 수행합니다. Actions 탭의 `Update holdings JSON`
워크플로는 수동 실행해도 daily 증분 조회만 수행합니다.
...
- 배가명: `shelfName`
- 등록일: `registeredAt`
- 등록번호: `registrationNumber` 선택 필드

신청자명, 회원번호, 전화번호, 내부 검토 메모, 예산 관련 메모, 담당자 메모, 민원 관련 정보는 기본 포함하지 않습니다. 등록번호가 정보나루 응답에서 확인되면 `registrationNumber` 선택 필드로만 포함합니다. 확인되지 않으면 빈 문자열이거나 생략될 수 있으며, 웹앱은 이 필드가 없어도 동작합니다.
...
- JSON 백업 다운로드
- 도움말과 한국어 오류 안내
- Python GUI 최초 전체 수집 구조
- 매일 GitHub Actions 증분 소장목록 갱신 구조
